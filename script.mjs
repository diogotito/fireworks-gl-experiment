...
import particleProto from "./Particle.mjs";
...
let particleShader = new G.Shader(
  gl,
  fireworks_vs.text.trim(),
  fireworks_fs.text.trim(),
);
particleShader.use();

const NUM_PARTICLES = 6_000;
const theArray = new Float32Array(particleProto.NUM_ELEMENTS * NUM_PARTICLES);
for (let offset = 0; offset < theArray.length; offset += particleProto.NUM_ELEMENTS) {
  Object.assign(Object.create(particleProto), {
    lifetime: 5,
    timeOffset: Math.random() * 5,
    startPos: G.Vec2.polar(Math.random() * Math.PI * 2, 5).add_xy(0, 10),
    startSize: 20 + 80 * Math.random(),
    startVel: G.Vec2.polar(
      Math.random() * Math.PI,
      6 + 6 * Math.random(),
    ).add_xy(0, 15 * Math.random()),
    acc: G.vec2(2 * Math.random() - 1, -9.81 - 10 * Math.random()),
    startColor: [
      new G.RGBA(0.95, 0.04, 0.1, 1.0),
      new G.RGBA(0.1, 0.85, 0.1, 1.0),
      new G.RGBA(0.1, 0.04, 0.85, 1.0),
      new G.RGBA(0.1, 0.85, 0.85, 1.0),
      new G.RGBA(0.95, 0.04, 0.85, 1.0),
      new G.RGBA(0.95, 0.85, 0.1, 1.0),
    ][Math.floor(Math.random() * 6)],
    endColor: new G.RGBA(0.5, 0.5, 0.5, 0.9),
  }).writeTo(theArray, offset);
}

let manyParticles = new Float32Array(particles.flatMap((p) => p.toArray()));
